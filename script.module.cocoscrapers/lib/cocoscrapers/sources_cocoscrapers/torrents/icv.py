...
class source(BaseTorrentScraper):
	...
	def _parse_file(file):
		try:
			...
			if not name:
				return None
			try:
				seeders = int(file.get('_meta', {}).get('seeders', 0))
			except:
				seeders = 0
			try:
				sb = int(hints.get('videoSize') or file.get('size') or 0)
				if sb >= 1024 ** 3:
	...
	def sources(self, data, hostDict):
		...
		try:
			...
			source_utils.scraper_error('ICV')
		finally:
			if is_tv:
				self._queue.put_nowait(files)
				self._queue.put_nowait(files)

		log_utils.log('ICV: %s raw results for "%s"' % (len(files), self.title))
		for file in files:
			try:
				...
				self._append_result(self._build_result('icv', hash, name, name_info, magnet, seeders, dsize, isize))
			except:
				source_utils.scraper_error('ICV')
	...
	def sources_packs(self, data, hostDict, search_series=False, total_seasons=None, bypass_filter=False):
		...
		try:
			...
			source_utils.scraper_error('ICV')
			self._log_stats('ICV', pack=True)
			return self._results

		log_utils.log('ICV packs: %s raw results for "%s"' % (len(files), self.title))
		for file in files:
			try:
				parsed = self._parse_file(file)
				if not parsed: continue
				...
				self._append_result(self._build_pack_result(
					'icv', hash, name, name_info, magnet, seeders, dsize, isize,
