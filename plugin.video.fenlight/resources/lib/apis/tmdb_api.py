...
def tmdb_movie_keyword_results(tmdb_id, page_no):
	...
	url = '%s/discover/movie?api_key=%s&language=it-IT&with_keywords=%s&page=%s' % (base_url, api_key, tmdb_id, page_no)
...
def tmdb_tv_keyword_results(tmdb_id, page_no):
	...
	url = '%s/discover/tv?api_key=%s&language=it-IT&with_keywords=%s&page=%s' % (base_url, api_key, tmdb_id, page_no)
	return lists_cache_object(get_data, string, url)
...
def tmdb_movies_popular(page_no):
	...
	url = '%s/movie/popular?api_key=%s&language=it-IT&region=US&with_original_language=en&page=%s' % (base_url, api_key, page_no)
	return lists_cache_object(get_data, string, url)
...
def tmdb_movies_popular_today(page_no):
	...
	url = '%s/trending/movie/day?api_key=%s&language=it-IT&region=US&with_original_language=en&page=%s' % (base_url, api_key, page_no)
...
def tmdb_movies_blockbusters(page_no):
	...
	url = '%s/discover/movie?api_key=%s&language=it-IT&region=US&with_original_language=en&sort_by=revenue.desc&page=%s' % (base_url, api_key, page_no)
...
def tmdb_movies_in_theaters(page_no):
	...
	url = '%s/movie/now_playing?api_key=%s&language=it-IT&region=US&with_original_language=en&page=%s' % (base_url, api_key, page_no)
	return lists_cache_object(get_data, string, url)
...
def tmdb_movies_upcoming(page_no):
	...
	url = '%s/discover/movie?api_key=%s&language=it-IT&region=US&with_original_language=en&release_date.gte=%s&release_date.lte=%s&with_release_type=3|2|1&page=%s' \
...
def tmdb_movies_latest_releases(page_no):
	...
	url = '%s/discover/movie?api_key=%s&language=it-IT&region=US&with_original_language=en&release_date.gte=%s&release_date.lte=%s&with_release_type=4|5|6&page=%s' \
...
def tmdb_movies_premieres(page_no):
	...
	url = '%s/discover/movie?api_key=%s&language=it-IT&region=US&with_original_language=en&release_date.gte=%s&release_date.lte=%s&with_release_type=1|3|2&page=%s' \
							% (base_url, api_key, previous_date, current_date, page_no)
	return lists_cache_object(get_data, string, url, expiration= EXPIRY_1_DAY)
...
def tmdb_movies_genres(genre_id, page_no):
	...
	url = '%s/discover/movie?api_key=%s&with_genres=%s&language=it-IT&region=US&with_original_language=en&release_date.lte=%s&page=%s' \
...
def tmdb_movies_languages(language, page_no):
	...
	url = '%s/discover/movie?api_key=%s&language=it-IT&with_original_language=%s&release_date.lte=%s&page=%s' \
...
def tmdb_movies_certifications(certification, page_no):
	...
	url = '%s/discover/movie?api_key=%s&language=it-IT&region=US&with_original_language=en&certification_country=US&certification=%s&sort_by=%s&release_date.lte=%s&page=%s' \
...
def tmdb_movies_year(year, page_no):
	...
	url = '%s/discover/movie?api_key=%s&language=it-IT&region=US&with_original_language=en&certification_country=US&primary_release_year=%s&page=%s' \
...
def tmdb_movies_decade(decade, page_no):
	...
	url = '%s/discover/movie?api_key=%s&language=it-IT&region=US&with_original_language=en&primary_release_date.gte=%s' \
...
def tmdb_movies_recommendations(tmdb_id, page_no):
	...
	url = '%s/movie/%s/recommendations?api_key=%s&language=it-IT&region=US&with_original_language=en&page=%s' % (base_url, tmdb_id, api_key, page_no)
	return lists_cache_object(get_data, string, url)
...
def tmdb_movies_search_filtered(query, page_no):
	def _get_filtered(url):
		data = get_tmdb(url).json()
		results = []
		for item in data.get('results', []):
			release_date = item.get('release_date', '')
			if release_date and release_date <= today and item.get('vote_count', 0) >= 50:
				results.append(remove_keys(item, tmdb_dict_removals))
	...
	url = '%s/search/movie?api_key=%s&language=it-IT&include_adult=%s&query=%s&page=%s' % (base_url, api_key, meta_filter, query, page_no)
	return lists_cache_object(_get_filtered, string, url)

def tmdb_movies_companies(company_id, page_no):
	api_key = tmdb_api_key()
	if api_key in empty_setting_check: return no_api_key()
	string = 'tmdb_movies_companies_%s_%s' % (company_id, page_no)
	url = '%s/discover/movie?api_key=%s&language=it-IT&region=US&with_original_language=en&with_companies=%s&page=%s' \
...
def tmdb_tv_popular(page_no):
	...
	url = '%s/tv/popular?api_key=%s&language=it-IT&region=US&with_original_language=en&page=%s' % (base_url, api_key, page_no)
	return lists_cache_object(get_data, string, url)
...
def tmdb_tv_popular_today(page_no):
	...
	url = '%s/trending/tv/day?api_key=%s&language=it-IT&region=US&with_original_language=en&page=%s' % (base_url, api_key, page_no)
	return lists_cache_object(get_data, string, url, expiration= EXPIRY_1_DAY)
...
def tmdb_tv_premieres(page_no):
	...
	url = '%s/discover/tv?api_key=%s&language=it-IT&region=US&with_original_language=en&include_null_first_air_dates=false&first_air_date.gte=%s&first_air_date.lte=%s&page=%s' \
							% (base_url, api_key, previous_date, current_date, page_no)
	return lists_cache_object(get_data, string, url, expiration= EXPIRY_1_DAY)
...
def tmdb_tv_airing_today(page_no):
	...
	url = '%s/tv/airing_today?api_key=%s&language=it-IT&region=US&with_original_language=en&page=%s' % (base_url, api_key, page_no)
	return lists_cache_object(get_data, string, url, expiration= EXPIRY_1_DAY)
...
def tmdb_tv_on_the_air(page_no):
	...
	url = '%s/tv/on_the_air?api_key=%s&language=it-IT&region=US&with_original_language=en&page=%s' % (base_url, api_key, page_no)
	return lists_cache_object(get_data, string, url, expiration= EXPIRY_1_DAY)
...
def tmdb_tv_upcoming(page_no):
	...
	url = '%s/discover/tv?api_key=%s&language=it-IT&region=US&with_original_language=en&first_air_date.gte=%s&first_air_date.lte=%s&page=%s' \
							% (base_url, api_key, current_date, future_date, page_no)
	return lists_cache_object(get_data, string, url, expiration= EXPIRY_1_DAY)
...
def tmdb_tv_genres(genre_id, page_no):
	...
	url = '%s/discover/tv?api_key=%s&with_genres=%s&language=it-IT&region=US&with_original_language=en&include_null_first_air_dates=false&first_air_date.lte=%s&page=%s' \
							% (base_url, api_key, genre_id, get_current_date(), page_no)
	return lists_cache_object(get_data, string, url, expiration= EXPIRY_1_WEEK)
...
def tmdb_tv_languages(language, page_no):
	...
	url = '%s/discover/tv?api_key=%s&language=it-IT&include_null_first_air_dates=false&with_original_language=%s&first_air_date.lte=%s&page=%s' \
							% (base_url, api_key, language, get_current_date(), page_no)
	return lists_cache_object(get_data, string, url, expiration= EXPIRY_1_WEEK)
...
def tmdb_tv_networks(network_id, page_no):
	...
	url = '%s/discover/tv?api_key=%s&language=it-IT&region=US&with_original_language=en&include_null_first_air_dates=false&with_networks=%s&first_air_date.lte=%s&page=%s' \
...
def tmdb_tv_year(year, page_no):
	...
	url = '%s/discover/tv?api_key=%s&language=it-IT&region=US&with_original_language=en&include_null_first_air_dates=false&first_air_date_year=%s&page=%s' \
							% (base_url, api_key, year, page_no)
	return lists_cache_object(get_data, string, url, expiration= EXPIRY_1_WEEK)
...
def tmdb_tv_decade(decade, page_no):
	...
	url = '%s/discover/tv?api_key=%s&language=it-IT&region=US&with_original_language=en&include_null_first_air_dates=false&first_air_date.gte=%s' \
...
def tmdb_tv_recommendations(tmdb_id, page_no):
	...
	url = '%s/tv/%s/recommendations?api_key=%s&language=it-IT&region=US&with_original_language=en&page=%s' % (base_url, tmdb_id, api_key, page_no)
	return lists_cache_object(get_data, string, url)
...
def tmdb_tv_search_filtered(query, page_no):
	def _get_filtered(url):
		...
		for item in data.get('results', []):
			...
			if first_air_date and first_air_date <= today and item.get('vote_count', 0) >= 50:
				results.append(remove_keys(item, tmdb_dict_removals))
		data['results'] = results
		return data
	api_key = tmdb_api_key()
...
def season_episodes_details(tmdb_id, season_no, language='en'):
	api_key = tmdb_api_key()
	if api_key in empty_setting_check: return no_api_key()
	try:
		url = '%s/tv/%s/season/%s?api_key=%s&language=%s&append_to_response=credits' % (base_url, tmdb_id, season_no, api_key, language)
		return get_tmdb(url).json()
	except: return None
